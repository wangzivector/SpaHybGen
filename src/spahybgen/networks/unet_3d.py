...
class UpConv3DBlock(nn.Module):
    ...
    def __init__(
        ...
        if last_layer:
            assert num_classes is not None, "num_classes required when last_layer=True"
...
class UNet3D(nn.Module):
    ...
    def __init__(
        ...
        in_channels: int = 1,
        augment: bool = False,
        voxel_discreteness: int = 80,
        orientation: Literal["quat", "so3", "R6d"] = "quat",
    ) -> None:
        super(UNet3D, self).__init__()
        self.orientation = orientation
        level_channels, bottleneck_channel = [16, 32, 64], 128
        if augment:
            level_channels, bottleneck_channel = [32, 64, 128], 256
        ...
        if self.augment_heads:
            self.conv_quat_a = nn.Conv3d(
                in_channels=level_1_chnls,
                out_channels=level_1_chnls // 2,
                kernel_size=(3, 3, 3),
                padding=(1, 1, 1),
            )
            self.acti_quat = nn.Tanh()
            self.conv_so3_a = nn.Conv3d(
                in_channels=level_1_chnls,
                out_channels=level_1_chnls // 2,
                kernel_size=(3, 3, 3),
                padding=(1, 1, 1),
        ...
        self.conv_score = nn.Conv3d(
            in_channels=level_1_chnls // (2 if self.augment_heads else 1),
            out_channels=1,
            kernel_size=(1, 1, 1),
        )
        self.conv_wren = nn.Conv3d(
            in_channels=level_1_chnls // (2 if self.augment_heads else 1),
            out_channels=1,
            kernel_size=(1, 1, 1),
        )

        if self.orientation == "quat":
            rot_head_size = 4
        elif self.orientation == "so3":
            rot_head_size = 3
        elif self.orientation == "R6d":
            rot_head_size = 6
        self.conv_rot = nn.Conv3d(
            in_channels=level_1_chnls // (2 if self.augment_heads else 1),
            out_channels=rot_head_size,
            kernel_size=(1, 1, 1),
        )
    ...
    def forward(self, input: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        out_rot = (
            F.normalize(self.conv_rot(out_rot_a), dim=1)
            if self.orientation == "quat"
            else self.conv_rot(out_rot_a)
        )
